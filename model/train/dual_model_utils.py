...
def load_pretrained_weights(model, pretrained_model_path, device, submodule_key, logger=None):
    log = logger if logger is not None else logging.getLogger()
    flag = False  # Indicates successful loading
    desc = None  # Description of the loading outcome

    def adjust_key(k, prefix='bert.', submodule='SciBERT'):
        return f"{prefix}{k.replace(submodule + '.', '')}"

    if os.path.isfile(pretrained_model_path):
        log.info(f"=> Loading checkpoint '{pretrained_model_path}'")
        checkpoint = torch.load(pretrained_model_path, map_location=device)

        if 'model_state_dict' in checkpoint:
            full_state_dict = checkpoint['model_state_dict']
            processed_state_dict = {k[7:] if k.startswith('module.') else k: v for k, v in full_state_dict.items()}

            if submodule_key == 'SciBERT':
                submodule_dict = {adjust_key(k, 'bert.', submodule_key): v for k, v in processed_state_dict.items() if k.startswith(submodule_key)}
            else:
                submodule_dict = {k.replace(submodule_key + '.', ''): v for k, v in processed_state_dict.items() if k.startswith(submodule_key)}

            filtered_state_dict = {k: v for k, v in submodule_dict.items() if not k.startswith('classifier.')}

            if filtered_state_dict:
                missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)
                log.info(f"Loaded pretrained weights for '{submodule_key}'. Missing keys: {missing_keys}, Unexpected keys: {unexpected_keys}")
                desc = f"Loaded weights for '{submodule_key}' from '{pretrained_model_path}' with missing keys: {missing_keys}, unexpected keys: {unexpected_keys}"
                flag = True
            else:
                log.error(f"No weights found for the submodule '{submodule_key}' after processing.")
                desc = "No weights found for the submodule."
        else:
            log.error("No 'model_state_dict' found in the checkpoint.")
            desc = "No 'model_state_dict' found in the checkpoint."
    else:
        log.error(f"=> No checkpoint found at '{pretrained_model_path}'")
        desc = "No checkpoint found at the specified path."

    return flag, desc


def load_pretrained_model2(model, pretrained_model_path, device, submodule_key, logger=None):
    """
    Load a pretrained model and extract weights for a specific submodule after removing 'module.' prefix.

    Args:
        model: The model or submodule to which the weights will be loaded.
        pretrained_model_path: Path to the pretrained model checkpoint.
        device: The device to map the loaded model.
        submodule_key: Key for the submodule's weights in the loaded model.
        logger: Optional logger for logging information (default: None, which uses the logging module).
    """
    log = logger if logger is not None else logging.getLogger()
    flag = False  # Indicates successful loading
    desc = None  # Description of the loading outcome
    ...
    if os.path.isfile(pretrained_model_path):
        ...
        loaded_model = torch.load(pretrained_model_path, map_location=device)

        # Access the entire state dictionary from the loaded model
        if isinstance(loaded_model, torch.nn.Module):
            # Assuming the loaded model is a PyTorch module
            model_state_dict = loaded_model.state_dict()
        else:
            # If the loaded object is already a state dict
            model_state_dict = loaded_model

        # Remove 'module.' prefix from all keys in the state dict
        processed_state_dict = {k[7:] if k.startswith('module.') else k: v for k, v in model_state_dict.items()}

        # Check for the specific submodule and filter its weights
        submodule_dict = {k[len(submodule_key) + 1:]: v for k, v in processed_state_dict.items() if
                          k.startswith(submodule_key)}

        # 移除全连接层不匹配的权重
        filtered_state_dict = {k: v for k, v in submodule_dict.items() if not k.startswith('fc.')}

        if submodule_dict:
            # Load the processed state dictionary into the target model submodule
            missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)
            log.info(
                f"Loaded pretrained weights for '{submodule_key}'. Missing keys: {missing_keys}, Unexpected keys: {unexpected_keys}")
            desc = f"[resume model info] Loaded weights for '{submodule_key}' from '{pretrained_model_path}' with missing keys: {missing_keys}, unexpected keys: {unexpected_keys}"
            flag = True
        else:
            log.error(f"No submodule '{submodule_key}' found in the processed model.")
            desc = f"No submodule '{submodule_key}' found in the processed model."
    else:
        log.info(f"=> No checkpoint found at '{pretrained_model_path}'")
        desc = "No checkpoint found at the specified path."
